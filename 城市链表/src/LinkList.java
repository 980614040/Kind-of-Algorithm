...
public class LinkList implements IList{
    ...
    //向单链表插入数据
    public void insert(int index, Object data,Object position) throws Exception {
        Node preNode = head;
        int preIndex;
        ...
        Node newNode = new Node(data,position);		//创建新插入的结点对象
    ...
    //获取位置index处的数据
    public Object getposition(int index) throws IndexOutOfBoundsException {
    ...
    //按名字查找对应的角标
    public int indexOfname(Object data) {
        Node nextNode = head.next;
        int index;
    ...
    //按坐标查找对应的角标
    public int indexOfposition(Object position) {
        Node nextNode = head.next;
        int index;
        for(index=0;nextNode!=null;index++) {
            if(Integer.parseInt(nextNode.position.toString())<=Integer.parseInt(position.toString()))
                return index;
        }
        return -1;
    }
    ...
    public void display() {
        ...
        head.data="------";
        head.position="------";
        root=new Node("城市名","坐标",head);// 设置节点关系// 设置节点关系
        // 第二步：根据节点关系取出所有数据
        Node currentNode = root;                  // 当前从根节点开始读取
        while (currentNode != null) {                        // 当前节点存在数据
            System.out.println(currentNode.data+" "+currentNode.position);
